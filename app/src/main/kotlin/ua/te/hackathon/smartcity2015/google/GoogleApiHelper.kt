...
class GoogleApiHelper(context: Context) : GoogleApiClient.OnConnectionFailedListener {
  ...
  fun startGoogleRegistration(activity: FragmentActivity) {
    ...
    activity.startActivityForResult(signInIntent, GoogleApiHelper.Companion.GOOGLE_SIGN_IN_CODE)
  }
  ...
  fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent) {
    if (requestCode == GoogleApiHelper.Companion.GOOGLE_SIGN_IN_CODE) {
  ...
  private fun handleGoogleSignInResult(result: GoogleSignInResult?) {
    if (result == null) {
      val googleAuthResult = GoogleApiHelper.GoogleAuthResult(
          0,
    ...
    if (result.isSuccess) {
      val googleAuthResult = GoogleApiHelper.GoogleAuthResult(result.signInAccount!!)
      EventBus.getDefault().postSticky(googleAuthResult)
    } else {
      val googleAuthResult = GoogleApiHelper.GoogleAuthResult(
  ...
  override fun onConnectionFailed(connectionResult: ConnectionResult) {
    Logger.w(GoogleApiHelper.Companion.LOG_TAG, "Cannot connect to Google account", connectionResult)
    val googleAuthResult = GoogleApiHelper.GoogleAuthResult(0, context.getString(R.string.error_registration_google))
    EventBus.getDefault().postSticky(googleAuthResult)
  }
  ...
  class GoogleProfile(account: GoogleSignInAccount) {
    ...
    init {
      ...
      this.imageUrl = if (account.photoUrl != null) account.photoUrl!!.toString() + GoogleApiHelper.Companion.IMAGE_SIZE else null
  ...
  companion object {
    ...
    fun logOut() {
      ...
      val googleApiClient = GoogleApiClient.Builder(Injector.context!!).addApi(Auth.GOOGLE_SIGN_IN_API).build()
      ...
      googleApiClient.registerConnectionCallbacks(object : GoogleApiClient.ConnectionCallbacks {
        ...
        override fun onConnectionSuspended(i: Int) {
          Logger.w(GoogleApiHelper.Companion.LOG_TAG, "Google Api Connection suspended")
        }
      })
